...
const Navbar = () => {
  ...
  return (
    <div className='fixed w-full h-[60px] flex items-center md:grid md:grid-cols-3 md:gap-4 px-4 bg-white z-10'>
        ...
        <div>
            <h1 className={`#2F3035 text-[20px] leading-[23px] ml-[40px] mt-[0.5px] ${styles.heading}`}>SACCOM</h1>
        </div>

        

        {/* mobile-view-navbar */}
        <ul className={`${toggle ? 'block' : 'hidden'} w-3/4 bg-white absolute top-0 left-0 h-screen md:hidden flex flex-col justify-center   ${styles.mobile}`}>
            <li className='text-[#04050C] leading-[20px] text-[16px] flex px-[24px] py-[16px] font-EuclidCircularA font-medium'>
        ...
        <div className='hidden md:flex lg:w-[400px] md:w-[250px] h-full items-center justify-center '>
           <input type="text" className='relative w-[250px] lg:w-[400px] rounded-[40px] h-[40px] border-solid border-2 border-[#54555B] placeholder-[#B3B4BB] pl-[16px]' placeholder='Search through hostels' />
        ...
        <div className=' hidden md:flex flex-col w-full h-full items-center justify-center'>
            <div className=' w-[150px] lg:w-[150px] flex justify-evenly rounded-[40px] items-center h-[40px] border-solid border-2 border-[#54555B] '>
                <p onMouseOver={handleMouseOver} onMouseOut={handleMouseOut} className='w-[18px] h-[16px] mr-[3px] cursor-pointer '>
            ...
            <div>
                {lgtoggle && (
                <div className=' absolute  bg-blue-900 md:right-[80px] lg:right-[150px] h-full w-[400px] '>
